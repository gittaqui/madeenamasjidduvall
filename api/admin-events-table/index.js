const { ensureEventsTableExists, getEventsTable } = require('../shared/eventsTable');
...
module.exports = async function(context, req){
  ...
  if(method === 'POST'){
    ...
    const dateVal = (body.date||now.substring(0,10)).substring(0,10);
    // Auto-generate id if missing or invalid
    if(!id || !validateId(id)){
      const baseTitle = (body.title||'event').toLowerCase();
      let slug = baseTitle
        .replace(/&/g,' and ')
        .replace(/['`]/g,'')
        .replace(/[^a-z0-9]+/g,'-')
        ...
        .substring(0,60);
      if(!slug) slug = 'event';
      // ensure contains date
      if(!slug.includes(dateVal)) slug = slug + '-' + dateVal;
      // ensure starts with alnum
      if(!/^[a-z0-9]/.test(slug)) slug = 'e-' + slug;
      // enforce regex by trimming invalid trailing chars
      ...
      while(true){
        let exists = false;
        try { await eventsTable.getEntity('event', candidate); exists = true; } catch{}
        if(!exists) { id = candidate; break; }
        candidate = slug + '-' + counter; counter++; if(counter>50){ id = candidate; break; }
      }
    }
    if(!validateId(id)) return context.res = { status:400, body:{ error:'invalid_generated_id', id } };
    // Final pre-flight check (should be unique now)
    let preExisting = null; try { preExisting = await eventsTable.getEntity('event', id); } catch{}
    if(preExisting){
    ...
    try { await eventsTable.createEntity(entity); return context.res = { status:201, body:{ ok:true, created:id, autoGenerated: !(body.id) } }; }
    catch(e){
      const msg = e.message || String(e);
      const stackTop = (e.stack||'').split('\n').slice(0,5);
      // Distinguish missing table vs real conflict
      if(e.statusCode === 404 || /table not found/i.test(msg)){
        return context.res = { status:500, body:{ error:'events_table_missing', detail: msg, stack: stackTop } };
      }
      let existing = null; try { existing = await eventsTable.getEntity('event', id); } catch{}
      const isConflict = existing != null || /EntityAlreadyExists/i.test(msg) || e.statusCode === 409;
      if(isConflict){
        return context.res = { status:409, body:{ error:'exists', id, existing: existing ? { title: existing.title, date: existing.date, time: existing.time, published: existing.published, updatedUtc: existing.updatedUtc, createdUtc: existing.createdUtc } : null, stack: stackTop } };
      }
      return context.res = { status:500, body:{ error:'create_failed', detail: msg, code: e.code, statusCode: e.statusCode, stack: stackTop } };
    }
  }
  if(method === 'PUT'){
    const body = req.body || {};
