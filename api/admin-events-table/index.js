  ...
  if(method === 'POST'){
    ...
    try { await eventsTable.createEntity(entity); return context.res = { status:201, body:{ ok:true, created:id, autoGenerated: !(body.id) } }; }
      ...
      if(e.statusCode === 404 || /table not found/i.test(msg)){
        return context.res = { status:500, body:{ error:'events_table_missing', detail: msg } };
      }
      ...
      if(isConflict){
        return context.res = { status:409, body:{ error:'exists', id, existing: existing ? { title: existing.title, date: existing.date, time: existing.time, published: existing.published, updatedUtc: existing.updatedUtc, createdUtc: existing.createdUtc } : null } };
      }
      return context.res = { status:500, body:{ error:'create_failed', detail: msg } };
    }
  }
  if(method === 'PUT'){
    const body = req.body || {};
